...
function login(callback) {
	Username = prompt('Enter name (wala, felix, or ma):');
	//send login to server so that server known this person is available!
	if (nundef(DB.settings.loggedIn)) DB.settings.loggedIn = [];
	DB.settings.loggedIn.push(Username);
	console.log('saving db', DB)
	dbSave('boardGames', callback);
}
